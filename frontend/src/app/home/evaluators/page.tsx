    ...
    <div className="animate-fade-in-bottom flex flex-col w-full max-sm:max-w-none">
      <div className="select-none flex flex-col justify-center items-center w-full h-full">
        ...
        <p className="text-center">
          Create a new evaluator or select an existing evaluator to get started.
        </p>
        <div className="flex flex-wrap justify-center mt-7">
          <div className="bg-base-300 rounded-lg p-4 hover:bg-base-200 max-w-xs m-2">
            ...
            <p className="text-sm opacity-70">
              Leverage cutting-edge AI for accurate and efficient grading.
            </p>
          </div>
          <div className="bg-base-300 rounded-lg p-4 hover:bg-base-200 max-w-xs m-2">
            <p className="font-semibold text-md mb-2">
              📊 Detailed Result Insights
            </p>
            <p className="text-sm opacity-70">
              Explore detailed insights for a holistic view of student
              performance.
            </p>
          </div>
          <div className="bg-base-300 rounded-lg p-4 hover:bg-base-200 max-w-xs m-2">
            ...
            <p className="text-sm opacity-70">
              Create, organize, and add students with ease.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="animate-fade-in-bottom flex flex-col w-full max-sm:max-w-none">
        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-1 mt-4 w-full max-w-lg">
            <p className="flex items-center font-semibold text-xl">
              ...
              {evaluators[selectedEvaluator]?.title}
            </p>
          </div>
          <div className="flex items-center">
            <p className="flex items-center text-sm mr-5">
              <FiBook className="mr-2" />{" "}
              {evaluators[selectedEvaluator]?.class?.subject}
            </p>
            <p className="flex items-center text-sm">
              <FiUsers className="mr-2" />{" "}
              {evaluators[selectedEvaluator]?.class?.name}{" "}
              {evaluators[selectedEvaluator]?.class?.section}
            </p>
          </div>
        </div>
      ...
      <div className="overflow-y-auto">
        ...
        <div className="flex flex-wrap">
          {evaluators[selectedEvaluator]?.questionPapers.map(
            (file: string, i: number) => {
              return (
                <label
                  key={i}
                  htmlFor="preview_modal"
                  onClick={() => setImgPreviewURL(file)}
                >
                  <img
                    key={i}
                    src={file}
                    className="cursor-pointer w-20 h-20 border object-cover rounded-md mr-2 mb-2"
                  />
                </label>
              );
            }
        ...
        <div className="flex flex-wrap">
          {evaluators[selectedEvaluator]?.answerKeys.map(
            (file: string, i: number) => {
              return (
                <label
                  key={i}
        ...
        <p className="flex items-center mb-1 mt-4">
            ...
            <div role="alert" className="alert shadow-lg">
              ...
              <div>
                ...
                <div className="text-xs">
                  You need to add students to the class to evaluate their answer
                  sheets.
                </div>
              </div>
              <label
                onClick={() => (window.location.href = "/home/classes")}
            ...
            students?.map((student: any, i: any) => (
              <div key={i} className="flex flex-col max-w-lg mb-4">
                <p className="flex items-center mb-1">
                  ...
                  {evaluationData[student?.rollNo] &&
                  answerSheets[i] &&
                  answerSheets[i]?.length >= 1 ? (
                    <span className="ml-2 flex items-center text-green-500 text-sm">
                      <FiCheck className="mr-2" /> Evaluated
                    </span>
                  ) : (
                    ""
                  )}
                      ...
                      return (
                        ...
                        >
                          {evaluating === student?.rollNo ? (
                            <div className="bg-white p-1 rounded-full absolute flex items-center text-sm">
                              <span className="mr-1 loading loading-spinner loading-sm"></span>
                              <p>Evaluating...</p>
                            </div>
                          ) : (
                            ""
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              updateEvaluation(
                                evaluators[selectedEvaluator]?._id,
                                answerSheets
                              );
                            }}
                          >
                          ...
                          >
                            <img
                              key={j}
                              src={file}
                              className="cursor-pointer w-40 h-40 border object-cover rounded-md mr-2 mb-2"
                            />
                          </label>
                        </div>
                      );
      ...
      {evaluating !== -1 ? (
        <div className="flex flex-col mb-5">
          <p className="mb-2">
            Evaluating Answer Sheets of {students[evaluating - 1]?.name}...
            (Student {evaluating} of {students.length})
          </p>
          <progress
      ...
      <div className="flex justify-center my-2 max-w-lg">
        <p className="flex items-center text-xs opacity-70 mr-1">
          <FiFileText className="mr-1" /> {limits?.evaluationLimit} evaluations
          left
        </p>
        <Link href="/shop">
      ...
      <div className="modal" role="dialog">
        <div className="modal-box">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              className="btn btn-square"
              onClick={() => window.open(imgPreviewURL)}
            >
              <FiExternalLink />
            </button>
          </div>
        ...
        <label className="modal-backdrop" htmlFor="preview_modal">
          Close
        </label>
      </div>
      ...
      <input
        type="checkbox"
        id="evaluationlimitexceed_modal"
        className="modal-toggle"
      />
      <div className="modal">
        <div className="modal-box">
          <h3 className="flex items-center font-bold text-lg">
            <FiInfo className="mr-1" /> Evaluation limit exceeded
          </h3>
          <p className="py-4">
            You have reached the maximum limit of evaluations.
            <br />
            You can purchase more evaluations from the shop.
          </p>
          <div className="modal-action">
            <label
              ref={(x) => (limitExceedModalRef.current = x)}
              htmlFor="evaluationlimitexceed_modal"
              className="btn"
            >
              Cancel
            </label>
        ...
        <label className="modal-backdrop" htmlFor="evaluationlimitexceed_modal">
          Cancel
        </label>
      </div>
    </div>
